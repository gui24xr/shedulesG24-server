export default class EstablishmentsService {
    ...
    getOwnerEstablishments = async (ownerId) => {
        try {
            const ownerEstablishments = await this.establishmentsRepository.find({ ownerId: ownerId }).populate('establishmentConfig')
            //console.log('Establecimientos del owner: ',ownerEstablishments)
            return ownerEstablishments.map(establishment => this.#getEstablishmentDTO(establishment))
        } catch (error) {
